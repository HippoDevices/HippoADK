...
/*
void ServoAnalogWrite(uint32_t ulPin, uint32_t ulValue) {
	...
	//PWM Frequency : 50 Hz,Timer counter clk:1MHz
	uint16_t TIM_Prescaler = 84-1;
	uint16_t TIM_ARR = 20000 - 1;
	uint16_t Duty_Cycle = (uint16_t)((ulValue * 100) / 20000);	
...
//	uint16_t Duty_Cycle = (uint16_t)((ulValue * 100) / 20000);
  ...
  if (!pinEnabled[ulPin]) {
	...
	else if(g_APinDescription[ulPin].ulTimerPeripheral == TIM12)
