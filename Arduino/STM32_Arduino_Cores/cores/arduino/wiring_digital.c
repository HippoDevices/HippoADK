...
extern void pinMode( uint32_t ulPin, uint32_t ulMode )
{
  ...
  uint8_t gpio_pinsource = g_APinDescription[ulPin].GPIO_PinSource;
  uint8_t gpio_afselection = g_APinDescription[ulPin].GPIO_AFSelection;
  ...
  if(ulMode == AF_OUTPUT_PUSHPULL)  //  PWM Alternate Function,  ADC does not execute this step
  GPIO_PinAFConfig(gpio_port, gpio_pinsource, gpio_afselection);
}
